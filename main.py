...
def get_ciba():
    url1 = "https://v1.hitokoto.cn/?c=f"
    url2 = "https://v1.hitokoto.cn/?c=j"
    headers = {
    ...
    q = get(url2, headers=headers)
    note_en = r.json()["hitokoto"]
    note_ch = q.json()["hitokoto"]
...
def send_message(to_user, access_token, region_name, weather, temp, wind_dir, note_ch, note_en, max_temp, min_temp,
    ...
    for key, value in birthdays.items():
        ...
        if birth_day == 0:
            birthday_data = "宝宝今天生日哦，生日快乐，我爱你！".format(value["name"], value["name"])
        else:
            birthday_data = "距离{}的破蛋日还有{}天".format(value["name"], birth_day)
        # 将生日数据插入data
